...
from openprompt.plms import T5TokenizerWrapper,load_plm,T5LMTokenizerWrapper
from openprompt.prompts import PrefixTuningTemplate,SoftTemplate
from tqdm import tqdm 
from transformers import (AdamW, get_linear_schedule_with_warmup,
						  RobertaConfig, RobertaModel, RobertaTokenizer, T5Config, T5ForConditionalGeneration, PLBartConfig, PLBartForConditionalGeneration,PLBartTokenizer)
...
def main(args):
	...
	# model_config = RobertaConfig.from_pretrained("./unixcoder")
	# plm = RobertaModel.from_pretrained("./unixcoder", config=model_config)
	# tokenizer = RobertaTokenizer.from_pretrained("./unixcoder")

	model_config = PLBartConfig.from_pretrained("./plbart")
	plm = PLBartForConditionalGeneration.from_pretrained("./plbart",config=model_config)
	tokenizer = PLBartTokenizer.from_pretrained("./plbart")
	WrapperClass = T5TokenizerWrapper

	# define template
	promptTemplate = SoftTemplate(model=plm, tokenizer=tokenizer,
										  text='{"placeholder":"text_a"} Summarization: {"mask"} ', initialize_from_vocab=True,
										   num_tokens=50)
